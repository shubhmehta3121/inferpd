...
scatter95_req_tput = js(list(zip(s95["qps"], s95["req_tput"])))
...
all_req_vals = s50["req_tput"] + s95["req_tput"] + s50["qps"] + s95["qps"]
...
.footnote{{font-family:'DM Mono',monospace;font-size:0.78rem;color:#94a0c8;margin-top:20px;padding:14px 16px;border:1px solid var(--border);border-radius:8px;background:#0a0a12;line-height:1.55}}
...
<header>
  ...
  <p class="meta">
    vLLM 0.18.0 · LMCacheMPConnector · ZMQ · LMCache 0.4.2 · 3× A100 SXM 80GB · BF16<br>
    1 prefiller → 2 decoders (1P2D) · disagg proxy + LMCache multiprocess server (see runbook for CLI)
  </p>
</header>
...
<div class="grid">
  ...
  <div class="card" style="grid-column:1/-1">
    ...
    <div class="leg">
      ...
      <span><div class="dot" style="background:var(--amber)"></div>P95 profile — long-context stress</span>
...
<div class="profiles">
  ...
  <div class="profile">
    ...
    <div class="row" style="flex-direction:column;align-items:stretch"><span class="cache-note">{info95["cache_note"]}</span></div>
  </div>
</div>
...
const p95_req_tput = {scatter95_req_tput};
const xMax      = {x_max};
...
new Chart('c5',{{type:'scatter',
  data:{{datasets:[line(p50_req_tput,BLUE),line(p95_req_tput,AMBER),diag(reqTputMax)]}},
  options:{{
    responsive:true, maintainAspectRatio:false,
    plugins:{{legend:{{display:false}},tooltip:{{backgroundColor:'#0f1018',borderColor:'#1a1d2e',borderWidth:1,titleFont:{{size:13}},bodyFont:{{size:12}}}}}},
    scales:{{
