#!/bin/bash
# ============================================================
# 00_setup.sh — vLLM 0.18.0 + FlashInfer + LMCache 0.4.2 + DeepGEMM
# ============================================================
set -euo pipefail
INITIAL_DIR="$(pwd)"

echo "============================================================"
echo " DeepSeek-V3.2-NVFP4 — LMCache MP stack (1P + 1D)"
echo " GPU: 8× B200 192GB (SM100 Blackwell) | CUDA 12.8 | PyTorch 2.8.0"
echo "============================================================"

NUM_GPUS=$(nvidia-smi --query-gpu=name --format=csv,noheader | wc -l)
echo "[INFO] Found $NUM_GPUS GPU(s)"
if [ "$NUM_GPUS" -lt 8 ]; then
  echo "[ERROR] This layout needs at least 8 GPUs (4 prefiller + 4 decoder TP ranks)."
  exit 1
fi
...
# FP4 MoE on Blackwell: FlashInfer wheels (paired with torch 2.8 / cu128)
uv pip install flashinfer-python --find-links https://flashinfer.ai/whl/cu128/torch2.8/
