...
echo " Rate steps:  15 → 30 → 60 req/s"
echo " Per step:    $NUM_PROMPTS_PER_RATE prompts"
...
for RATE in 15 30 60; do
  DURATION=$((NUM_PROMPTS_PER_RATE / RATE))
  echo ""
  echo "──────────────────────────────────────────────────────"
  echo " Rate: $RATE req/s | Prompts: $NUM_PROMPTS_PER_RATE | Duration: ~${DURATION}s"
  echo "──────────────────────────────────────────────────────"
  ...
  vllm bench serve \
    "${COMMON_ARGS[@]}" \
    --num-prompts "$NUM_PROMPTS_PER_RATE" \
    --request-rate "$RATE" \
    --result-dir "$RESULTS_DIR" \
