...
"""
LMCache MP disaggregated prefill proxy (disagg_prefill_mp pattern).

Routing (per incoming request):
  - One shared monotonic index selects a prefiller client and a decoder client.
  - With **1 prefiller** and **N decoders**, the prefiller index is always 0 and
    decoders rotate **round-robin** (0,1,...,N-1,0,...).
  - With **P prefillers** and **D decoders**, both sides rotate with the same
    index (paired round-robin).

LMCache KV is shared via the ZMQ multiprocess server; each vLLM process uses
LMCacheMPConnector with a distinct engine_id. This file does not use NIXL.
"""
# Standard
...
async def lifespan(app: FastAPI):
    ...
    logger.info(
        "Disagg proxy backends: %d prefiller(s) %s, %d decoder(s) %s "
        "(round-robin per request with shared index).",
        len(prefill_pairs),
        [f"http://{h}:{p}" for h, p in prefill_pairs],
        len(decoder_pairs),
        [f"http://{h}:{p}" for h, p in decoder_pairs],
    )
...
def parse_args():
    ...
    default_kv_wait = float(
        os.environ.get("DISAGG_PROXY_KV_STORE_WAIT_TIMEOUT_SEC", "120")
    )
    parser.add_argument(
        "--kv-store-wait-timeout-sec",
        type=float,
        default=default_kv_wait,
        help=(
            "Max seconds to wait for LMCache telemetry "
            "(request_store_finished) after the prefiller returns. "
            "Default from DISAGG_PROXY_KV_STORE_WAIT_TIMEOUT_SEC or 120."
        ),
    )
    ...
    parser.add_argument(
        "--num-decoders",
        type=int,
        default=int(os.environ.get("NUM_DECODERS", "1")),
        help="Decoder replica count; with a single --decoder-port P, uses P..P+N-1.",
    )
    parser.add_argument("--telemetry-port", type=int, default=5768)

    args = parser.parse_args()
    if args.num_decoders < 1:
        raise SystemExit("--num-decoders must be >= 1")
    if args.num_prefillers < 1:
        raise SystemExit("--num-prefillers must be >= 1")
    return args
...
def round_robin_pick_clients() -> tuple[ClientInfo, ClientInfo]:
    """Advance global index; pick prefiller and decoder by that index (round-robin each pool)."""
...
async def _handle_disagg_request(request: Request, endpoint: str):
    ...
    try:
        ...
        try:
            ...
            # Wait for the event to be signaled (KV store finished). Without a
            # bounded wait, a missing or delayed request_store_finished telemetry
            # blocks this handler forever (see doc/disagg-proxy-kv-store-wait-timeout.md).
            kv_wait_timeout = global_args.kv_store_wait_timeout_sec
            try:
                await asyncio.wait_for(event.wait(), timeout=kv_wait_timeout)
            except asyncio.TimeoutError:
                logger.error(
                    f"Request {request_id}: timed out after {kv_wait_timeout}s "
                    "waiting for request_store_finished telemetry (KV store signal)"
                )
                return JSONResponse(
                    content={
                        "error": {
                            "message": (
                                "Timed out waiting for LMCache KV store completion "
                                "(request_store_finished telemetry). "
                                "Increase --kv-store-wait-timeout-sec or fix "
                                "prefiller/LMCache/telemetry path."
                            ),
                            "type": "kv_store_wait_timeout",
                            "request_id": request_id,
                            "timeout_sec": kv_wait_timeout,
                        }
                    },
                    status_code=504,
                )
...
@app.get("/health")
async def health():
    """Liveness check for load balancers and scripts/05_health_check.sh."""
